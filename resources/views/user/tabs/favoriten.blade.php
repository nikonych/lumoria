<div class="flex flex-col space-y-12">
    <div class="">
        <div class="mb-5 flex space-x-2.5 items-center">
            ...
            <span class="bg-indigo-200 text-indigo-700 px-3.5 py-1.5 rounded-full text-xs">
                {{ $user->favoriteMovies()->count() }}
            </span>
        </div>
        <x-carousel-pagination :items="$user->favoriteMovies" :per-page="5">
            <div x-ref="container"
    ...
    <div class="">
        <div class="mb-5 flex space-x-2.5 items-center">
            ...
            <span class="bg-indigo-200 text-indigo-700 px-3.5 py-1.5 rounded-full text-xs">
                {{ $user->favoritePeople()->count() }}
            </span>
        </div>
        <x-carousel-pagination :items="$user->favoritePeople" :per-page="5">
            <div x-ref="container"
                 class="flex scrollbar-hide overflow-x-auto scroll-smooth snap-x snap-mandatory space-x-5">
                @foreach($user->favoritePeople as $person)
                    <div class="flex-shrink-0 w-full sm:w-1/3 lg:w-1/8">
