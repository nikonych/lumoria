<div x-data="{ open: false }" @click.outside="open = false" class="relative inline-block text-left {{ $class }}">
    <div class="relative">
        ...
        <div class="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
            ...
            @endif
            <svg x-bind:class="{ 'rotate-180': open }" viewBox="0 0 20 20" fill="currentColor"
                 class="-mr-1 size-5 text-indigo-400 transition-transform duration-200">
                <path
        ...
        <div class="py-1 max-h-64 overflow-y-auto">
            ...
            <div class="px-4 py-2">
                <div class="flex gap-2">
                    <input
                        type="text"
                        wire:model.live="newItemInput"
                        @keydown.enter.prevent="$wire.addNewItem()"
                        placeholder="{{$label}}"
                        class="w-full font-light text-sm rounded-sm bg-input-dark px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                    />
                    <x-base.button
