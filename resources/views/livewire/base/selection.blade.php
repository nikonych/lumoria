<div x-data="{
    ...
    <button @click="toggle()"
            type="button"
            class="inline-flex w-full font-light text-sm justify-between items-center gap-x-1.5 rounded-sm bg-input-dark px-3 py-1.5 focus:outline-none cursor-pointer">
            ...
            @foreach ($options as $option)
                <a href="#"
                   ...
                   class="block px-4 py-2 text-sm hover:bg-white/5 hover:text-white
                          {{ $option['value'] == $value ? 'bg-white/10 text-indigo-300' : '' }}">
                    {{ $option['text'] }}
