<div x-data="{ open: false }" @click.outside="open = false" class="relative inline-block text-left">
    ...
    <button @click="open = !open" type="button" class="inline-flex w-full font-light text-sm justify-center items-center gap-x-1.5 rounded-md bg-input-dark px-3 py-1.5 focus:outline-none cursor-pointer">
        {{-- Display the text from our computed property --}}
        {{ $this->selectedText }}
    ...
    {{-- Dropdown Menu (controlled by Alpine.js for a responsive UI without server round-trips) --}}
    <div
        x-show="open"
    ...
    >
        <div class="py-1">
            @foreach ($options as $option)
                {{-- When clicked, call the 'selectOption' method in the Livewire class --}}
                <a href="#"
                   wire:click.prevent="selectOption('{{ $option['value'] }}')"
                   @click="open = false" {{-- Also close the dropdown via Alpine --}}
